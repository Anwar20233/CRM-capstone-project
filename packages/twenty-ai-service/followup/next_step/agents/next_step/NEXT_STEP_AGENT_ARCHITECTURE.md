...
**Location:** `followup/next_step/agents/next_step/`
...
```
followup/next_step/agents/next_step/
  imports from:
        followup/next_step/context/schemas.py      <- DealContext (P1 contract)
        followup/next_step/events/schemas.py       <- FollowUpEvent, FollowUpEventType (P1 contract)
...
```python
from followup.next_step.agents.next_step import run_next_step_agent
...
**Mocking strategy:**
- LLM: `monkeypatch.setattr("followup.next_step.agents.next_step.next_step_agent.call_llm_json", fake)`
