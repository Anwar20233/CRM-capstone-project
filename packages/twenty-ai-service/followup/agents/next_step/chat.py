...
Lets you pick a preset scenario or describe your own deal, then runs
`run_next_step_agent` and prints the recommendations.

No API keys required — the LLM is mocked with canned recommendations
that change per scenario.  Set LLM_PROVIDER + your API key env vars
if you want real LLM output.

Run from packages/twenty-ai-service:

    python -m followup.agents.next_step.chat
"""
...
from followup.agents.next_step import next_step_agent as _module
from followup.agents.next_step.next_step_agent import run_next_step_agent
from followup.agents.next_step.schemas import NextStepLLMActionItem, NextStepLLMOutput
...
from followup.rag.service import RetrievedChunk
...
class _StubRetrieval:
    ...
    async def retrieve_documents(self, query: str, collection: CollectionName, top_k: int = 5):
        return [c for c in self._chunks if c.collection == collection][:top_k]


def _default_chunks() -> list[RetrievedChunk]:
    return [
        RetrievedChunk(
            content="Discovery: confirm the decision maker and document the pain point before proposing.",
            source="playbooks/Discovery.md",
            collection=CollectionName.SALES_PLAYBOOKS,
            score=0.9,
        ),
        RetrievedChunk(
            content="Proposal: send proposal once budget and need are confirmed.",
            source="playbooks/Proposal.md",
            collection=CollectionName.SALES_PLAYBOOKS,
            score=0.85,
        ),
        RetrievedChunk(
            content="BANT — Authority: a contact flagged is_decision_maker must be engaged.",
            source="bant.md",
...
def _mock_llm(actions: list[NextStepLLMActionItem], summary: str, confidence: float = 0.8):
    async def _fake(prompt: str, schema):
...
PRESETS: list[dict] = [
    {
        ...
        "description": "Acme Corp is in Discovery. The rep has an overdue task and no budget info yet.",
        ...
        "mock": _mock_llm(
            [
                NextStepLLMActionItem(
                    ...
                    suggested_crm_instruction="Create a task on opportunity opp-1001: Identify and engage the economic buyer",
                ),
                NextStepLLMActionItem(
                    ...
                    profile_fact_refs=[],
                    suggested_crm_instruction="Reschedule the overdue task on opportunity opp-1001",
                ),
                NextStepLLMActionItem(
                    ...
                    profile_fact_refs=["fact-1"],
                    suggested_crm_instruction="Create a task on opportunity opp-1001: Discuss and document budget range",
                ),
            ],
    ...
    },
    {
        ...
        "description": "Globex Inc is in Proposal stage. Budget ($75k-$90k) confirmed, decision maker known.",
        ...
        "mock": _mock_llm(
            [
                NextStepLLMActionItem(
                    ...
                    priority=1,
                    reasoning="All Proposal-stage gates are cleared: DM engaged, budget $75k-$90k, Q3 deadline.",
                    evidence=["Fact: bant_budget_status = confirmed $75k-$90k.", "Contact: Priya Shah is decision maker."],
                    profile_fact_refs=["fact-2", "fact-3"],
                    suggested_crm_instruction="Create a note on opportunity opp-2002: Proposal sent to Priya Shah",
                ),
                NextStepLLMActionItem(
                    ...
                    profile_fact_refs=[],
                    suggested_crm_instruction="Schedule a meeting on opportunity opp-2002 with Priya Shah to review the proposal",
                ),
            ],
            summary="Budget and authority are confirmed — send the proposal now and lock in a walkthrough call.",
            confidence=0.91,
        ),
    },
    {
        "title": "Discovery deal — meeting completed (post-intro call follow-up)",
        "description": "Acme Corp just had the first meeting. Rep needs next steps.",
        "loader": lambda: _load("next_step_context_discovery.json"),
        "event_type": FollowUpEventType.MEETING_COMPLETED,
        "mock": _mock_llm(
            [
                NextStepLLMActionItem(
                    ...
                    title="Send meeting summary and next steps",
                    description="Recap the pain points discussed and confirm next steps.",
                    priority=1,
                    reasoning="Best practice after any meeting: document outcomes and confirm momentum.",
                    evidence=["Timeline: intro call covered inventory reconciliation pain.", "No future meeting scheduled."],
                    profile_fact_refs=[],
                    suggested_crm_instruction="Create a task on opportunity opp-1001: Send meeting recap email to Jordan Lee",
                ),
                NextStepLLMActionItem(
                    action_type="qualify_budget",
                    title="Initiate budget conversation",
                    description="Budget was not discussed — probe in the next touchpoint.",
                    priority=2,
                    reasoning="BANT Budget is missing. The sooner it's qualified, the less time is wasted.",
                    evidence=["profile fact: bant_budget_status = missing."],
                    profile_fact_refs=["fact-1"],
                    suggested_crm_instruction="Create a task on opportunity opp-1001: Ask Jordan about budget in the next call",
                ),
            ],
            summary="Post-meeting: send a recap to keep momentum, then probe for budget.",
            confidence=0.77,
        ),
    },
    {
        ...
        "event_type": FollowUpEventType.OPPORTUNITY_STAGE_CHANGED,
        "mock": None,  # skip happens before LLM — no mock needed
    },
    {
    ...
    },
    {
        ...
        "description": "Opportunity has no company attached. Agent skips rather than hallucinate.",
...
def _build_custom_context() -> tuple[DealContext, FollowUpEventType]:
    ...
    company = CompanySnapshot(id="co-1", name=company_name, industry="Technology") if company_name else None

    contact_name = _ask("Primary contact name", "Alex Kim")
    is_dm = _ask("Is this contact a decision maker? (y/n)", "n").lower() == "y"
    contacts = [ContactSnapshot(id="ct-1", name=contact_name, role="Champion", is_decision_maker=is_dm)] if contact_name else []

    has_overdue = _ask("Add an overdue task? (y/n)", "n").lower() == "y"
    tasks = []
    if has_overdue:
        from followup.context.schemas import TaskSnapshot
        tasks = [TaskSnapshot(id="task-1", title="Follow up with client", status="open",
    ...
    timeline = []
    if timeline_note:
        timeline = [TimelineItem(id="tl-1", type="note", title="Note",
                                 summary=timeline_note, occurred_at=datetime.now(timezone.utc))]

    budget_known = _ask("Is the budget known? (y/n)", "n").lower() == "y"
    from followup.context.schemas import ProfileFact, FactCategory
    active_facts = []
    if budget_known:
        active_facts.append(ProfileFact(fact_id="f-1", category=FactCategory.DEAL_PROGRESSION,
                                        fact_key="bant_budget_status", value="confirmed"))
    else:
        active_facts.append(ProfileFact(fact_id="f-1", category=FactCategory.DEAL_PROGRESSION,
                                        fact_key="bant_budget_status", value="missing"))
    ...
    engagement = EngagementMetrics(days_since_last_activity=5, activity_count_14d=3,
                                   activity_count_prior_14d=4, has_future_meeting=False)
    pipeline_meta = PipelineMeta(stage_sla_days={stage: 14}, days_in_current_stage=7)
    ctx = DealContext(opportunity=opp, company=company, contacts=contacts, timeline=timeline,
                      tasks=tasks, meetings=[], pipeline_meta=pipeline_meta,
...
def _print_result(result) -> None:
    ...
    print(_c(f"\n[OK] {len(result.recommended_actions)} recommendation(s)  "
             f"(confidence {result.confidence:.0%})", _GREEN))
    ...
    for i, action in enumerate(result.recommended_actions, 1):
        ...
        for e in action.evidence:
            print(f"       Evidence   : • {e}")
        print(_c(f"       CRM Action : {action.suggested_crm_instruction}", _CYAN))
        print()
...
async def _run_scenario(ctx: DealContext, etype: FollowUpEventType, mock) -> None:
    ...
    if not USE_REAL_LLM and mock is not None:
        _module.call_llm_json = mock

    result = await run_next_step_agent(ctx, event, _StubRetrieval())
