# Follow-Up Intelligence Layer — Final Specification

> **Canonical spec — self-contained.** No other plan doc required.
> **Repo:** `packages/twenty-ai-service/FOLLOWUP_INTELLIGENCE_LAYER_FINAL_SPEC.md`
> **Notion:** [Final Specification (complete)](https://app.notion.com/p/37d5a414128c8120ab51db9a6345de81) ·
> [P1](https://app.notion.com/p/37d5a414128c818599e9f35e8d30e483) ·
> [P2](https://app.notion.com/p/37d5a414128c81eda7efc67d608c7ec2) ·
> [P3](https://app.notion.com/p/37d5a414128c81d48697f8d84bfa7eb5) ·
> [P4](https://app.notion.com/p/37d5a414128c81a4af48cc4786cd1401)
>
> **Team:** 4 developers · **Package:** `packages/twenty-ai-service/followup/`

## Quick navigation

| Person | Role | Jump to |
|--------|------|---------|
| **P1** | Orchestrator + Client Profile + API + schedulers | [Person 1](#person-1--follow-up-orchestrator--client-profile) |
| **P2** | Next Step Agent | [Person 2](#person-2--next-step-intelligence-agent) |
| **P3** | Risk Agent + Daily Sweep | [Person 3](#person-3--risk-scoring--notification-agent--daily-sweep) |
| **P4** | Drafting + Email Monitoring + UI | [Person 4](#person-4--drafting-agent--email-monitoring--ui) |

> **Golden rule:** Follow-Up **NEVER writes to CRM**. All writes: User Accept → `POST /followup/accept` → CRM Orchestrator → Writer Agent → agent-bridge.

---

## Part A — Architecture (v2 complete system)

This system has **four entry points** sharing one agent library and one Client Profile memory layer:

| Entry | Trigger | Endpoint / Schedule |
|-------|---------|---------------------|
| **A — CRM chat direct** | Rep asks in chat | `POST /followup/direct` |
| **B — Email monitoring** | Scheduled | Phase 1 hourly 8–18 TZ; Phase 2 every 15 min |
| **C — Daily risk sweep** | Scheduled | Daily ~6:00 workspace TZ |
| **D — CRM events (legacy)** | CRM change | `POST /followup/events` via BullMQ |

### Four entry point call graphs

**Entry A — CRM Orchestrator Route:**
```text
Sales Rep → CRM Orchestrator → delegate_to_agent("followup")
  → POST /followup/direct → run_direct_followup()
  → ClientProfileService.get_profile → load_deal_context(profile_primary)
  → check_calendar_availability → run_drafting_agent
  → persist → UI → Accept → POST /followup/accept → CRM Orchestrator → Writer
```

**Entry B — Email Monitoring Phase 1 (collect only):**
```text
Scheduler → fetch_inbound_since → match_email_to_opportunity
  → classify_email_intent → extract_email_entities → store_structured_email_event
```

**Entry B — Email Monitoring Phase 2 (act):**
```text
Scheduler → review_email_event → profile + context → action by intent
  → draft/rec/notify → mark processed
```

| Intent | Action |
|--------|--------|
| meeting_request | Calendar → draft reply |
| question | Draft response |
| objection | Recommendation + draft |
| info_share | Profile update only |
| follow_up | Draft + optional next_step |
| unrelated | Skip |

**Entry C — Daily Risk Sweep:**
```text
Scheduler → list active opportunities → run_risk_notification_agent
  → compare_score_to_previous → if threshold/delta → re-engagement draft + notify
  → update profile risk facts
```

**Entry D — CRM Event Route (preserved):**
```text
CRM change → NestJS listener → BullMQ → POST /followup/events
  → run_followup_pipeline → profile-first context → parallel agents → persist
```

### Client Profile memory layer

Persistent fact-based memory per opportunity — **not rebuilt each request**.

**Profile categories:** identity · communication_behavior · relationship_context · deal_progression · risk · derived_insights

**Every ProfileFact includes:** fact_id, category, fact_key, value, timestamp, source_reference, source_type, confidence, freshness_score, status (active/superseded/stale/retired)

**Update triggers:** email review · risk score update · CRM field change · draft acceptance · meeting completion · consolidation (daily)

**Source priority (conflicts):** crm_structured > email_extracted/meeting_summary/draft_acceptance > behavioral_inference/risk_computation

**Public APIs:**
```python
async def get_profile(opportunity_id: str, workspace_id: str) -> ClientProfile
async def get_active_facts(profile_id: UUID, category: FactCategory | None = None) -> list[ProfileFact]
async def apply_fact_updates(profile_id, candidates, trigger, trigger_reference_id) -> ProfileUpdateEvent
async def bootstrap_profile_from_crm(opportunity_id, workspace_id, user_id) -> ClientProfile
async def run_profile_consolidation(workspace_id: str) -> ConsolidationResult
```

### Updated DealContext (profile-primary)

```python
class DealContext(BaseModel):
    profile: ClientProfile
    active_facts: list[ProfileFact]
    context_provenance: Literal["profile_primary", "crm_fallback", "hybrid"]
    ...
    raw_fallback: RawCrmFallback | None
    loaded_at: datetime
```

```python
async def load_deal_context(
    opportunity_id, workspace_id, user_id, *,
    profile_primary: bool = True,
    fallback_on_stale: bool = True,
    stale_threshold_hours: int = 72,
) -> DealContext
```

### v2 schemas and tables (additions)

**StructuredEmailEvent** — table `structured_email_events` (no raw email body in primary record)

**ClientProfile** — table `client_profiles` (unique workspace_id + opportunity_id)

**ProfileFact** — table `client_profile_facts`

**ProfileUpdateEvent** — table `client_profile_update_events`

**RiskScoreSnapshot** — table `risk_score_snapshots` (source: event | daily_sweep)

**CalendarAvailabilityResult** — ephemeral; optional Redis cache TTL 15 min

### v2 API endpoints (additions)

| Method | Path | Purpose |
|--------|------|---------|
| POST | `/followup/direct` | CRM chat delegation (Entry A) |
| POST | `/followup/workflows/email/fetch` | Email Phase 1 cron |
| POST | `/followup/workflows/email/review` | Email Phase 2 cron |
| POST | `/followup/workflows/risk/sweep` | Daily risk sweep cron |
| POST | `/followup/workflows/profile/consolidate` | Daily profile consolidation |

**Preserved:** `POST /followup/events` · `GET /followup/health/{id}` · `GET/PATCH /followup/notifications` · `POST /followup/accept`

### v2 function contracts (additions)

```python
async def run_direct_followup(request: DirectFollowUpRequest) -> FollowUpRunResult
async def accept_followup(payload: AcceptFollowUpRequest) -> AcceptFollowUpResponse
async def run_email_fetch_workflow(workspace_id: str) -> EmailFetchResult
async def run_email_review_workflow(workspace_id: str, *, batch_size: int = 50) -> EmailReviewBatchResult
async def review_email_event(event_id: UUID) -> EmailReviewResult
async def run_daily_risk_sweep(workspace_id: str) -> RiskSweepResult
async def compare_score_to_previous(opportunity_id: str, new_score: int) -> RiskScoreSnapshot
async def check_calendar_availability(owner_user_id, workspace_id, proposed_times, *, duration_minutes=30) -> CalendarAvailabilityResult
```

### v2 folder structure (additions)

```text
followup/
├── profile/              # Client Profile Service (P1)
├── workflows/
│   ├── email/            # Phase 1 + Phase 2 (P4)
│   ├── risk_sweep/       # Daily sweep (P3)
│   └── direct/           # Entry A (P1)
├── calendar/             # availability.py (P1)
└── store/
    ├── email_repository.py
    └── profile_repository.py
```

### 4-week implementation timeline (v2)

| Week | Milestone |
|------|-----------|
| 1 | Profile tables + schemas frozen; agents on fixtures |
| 2 | Email Phase 1 + direct route; profile writes |
| 3 | Email Phase 2 + daily sweep; integration demo |
| 4 | Consolidation, caching, E2E hardening |

---

## Part B — Implementation guide (team ownership + Cursor prompts)

> Each developer jumps to their Person section. Section A = what you own. Section B = Cursor copy-paste prompt.
...
**How the 4 pieces connect (all entry points):**

```text
Entry A/B/C/D → [P1 Orchestrator + Client Profile] → profile-first DealContext
                              ↓
              ┌───────────────┼───────────────┐
              ↓               ↓               ↓
...
# Person 1 — Follow-Up Orchestrator + Client Profile

## A. What you own and what it does

You build the **central brain** of the Follow-Up system **and the Client Profile memory layer**. You do **not** build the three intelligence agents — you **call** them.

**v2 additions you own:**
- **Client Profile Service** (`followup/profile/`) — persistent fact memory, bootstrap, conflicts, consolidation
- **Entry A** — `POST /followup/direct` for CRM chat delegation
- **Scheduler host** — NestJS crons calling email fetch/review, risk sweep, profile consolidation
- **Calendar infra** — `check_calendar_availability()` for meeting requests
- **Profile-first** `load_deal_context()` — CRM is fallback only
...
**v2 addition:** Use `context.profile` and `context.active_facts` (relationship_context, deal_progression, derived_insights) in prompts. Add `profile_fact_refs[]` to each `RecommendedAction`. Handle email `objection` synthetic events from P4 Phase 2.
...
# Person 3 — Risk Scoring & Notification Agent + Daily Sweep
...
**v2 additions you own:**
- **Daily Risk Sweep** (`followup/workflows/risk_sweep/`) — runs ~6:00 workspace TZ on all active opportunities
- **`risk_score_snapshots`** table — trend, delta, threshold crossing detection
- **`compare_score_to_previous()`** — triggers re-engagement draft when `|delta| >= 10` or level crosses MEDIUM/HIGH
- Profile risk fact updates via P1 `apply_fact_updates` after each score
...
# Person 4 — Drafting Agent + Email Monitoring + UI
...
**v2 additions you own:**
- **Email Monitoring Phase 1** — fetch, match, classify, store structured events (hourly 8–18 TZ)
- **Email Monitoring Phase 2** — review unprocessed events, draft/rec/notify by intent (every 15 min)
- **Meeting-request drafts** using `CalendarAvailabilityResult` from P1
- **Re-engagement drafts** triggered by P3 daily sweep
- UI source badges (email / risk / chat / crm_event)
...
class FollowUpState(TypedDict):
    event: FollowUpEvent
    run_id: str
    entry_point: FollowUpEntryPoint | None
...
## Part C — Migration reference

### New in final spec
- Client Profile Service, email monitoring (2 phases), daily risk sweep, calendar integration
- Entry A `/followup/direct`, workflow cron endpoints, v2 storage tables

### Modified
- `DealContext` (profile-primary), `load_deal_context`, `followup_stub` → real delegation, `FollowUpState`

### Unchanged
- Accept flow, agent public signatures, CRM event route, LangGraph topology, v1 `followup_*` tables, RAG protocol

### Superseded documents
- `FOLLOWUP_INTELLIGENCE_LAYER_PLAN.md` — archived; use this file
- `FOLLOWUP_ORCHESTRATOR_V2_PLAN.md` — archived; merged here

---

## Appendix — P3 implementation kickoff (Person 3)

**Status:** Not started — `followup/` package does not exist yet.

### Week 1 deliverables (your ownership)

1. `followup/agents/risk/rules.py` — 7 rules, score cap 100, LOW/MEDIUM/HIGH levels
2. `followup/agents/risk/notifications.py` — max 2 notifications, 7-day dismiss dedupe
3. `followup/agents/risk/agent.py` — `run_risk_notification_agent(context, event, existing_notifications)`
4. `followup/workflows/risk_sweep/` — `compare_score_to_previous`, `run_daily_risk_sweep`
5. `followup/tests/test_risk_agent.py` — all 10 spec scenarios passing

### Prerequisite (coordinate with P1)

Import from P1 — do not redefine:

- `followup/context/schemas.py` → `DealContext`
- `followup/events/schemas.py` → `FollowUpEvent`

If P1 has not landed schemas yet, scaffold minimal copies matching the spec so your tests can run.
...
Switch Cursor to **Agent mode** and say: *"continue P3 implementation"*. Plan mode blocks Python file creation.
