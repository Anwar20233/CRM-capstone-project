...
import { UserMessageWithHighlights } from '@/ai/components/UserMessageWithHighlights';
...
export const AiChatMessage = ({
  ...
  const entitySpans = agentChatMessage.metadata?.entitySpans;
  const userMessageText = agentChatMessage.parts.find(
    (part) => part.type === 'text',
  )?.text;

  const shouldHighlightUserMessage =
    isUser && isDefined(entitySpans) && isDefined(userMessageText);

  return (
    <StyledMessageBubble isUser={isUser}>
      <StyledMessageContainer isUser={isUser}>
        <StyledMessageText isUser={isUser}>
          {shouldHighlightUserMessage ? (
            <UserMessageWithHighlights
              messageId={messageId}
